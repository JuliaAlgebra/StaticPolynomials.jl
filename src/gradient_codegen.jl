...
function generate_gradient(E, P, ::Type{T}, access_input) where T
    ...
    val, dvals = partial_derivatives!(exprs, E, P, T, nvars, 1, access_input)
    out = :($(val), SVector($(Expr(:tuple, map(first, dvals)...))))

    Expr(:block, exprs..., out)
end

function partial_derivatives!(exprs, E, P, ::Type{T}, nvar, nterm, access_input) where T
    m, n = size(E)

    # We only have one Term remaining. So we just evaluate the term and compute all
    # partial derivatives
    if n == 1
        return term(E, P, T, nterm, access_input)
    elseif m == 1
        return univariate!(exprs, E, P, T, nvar, nterm, access_input)
    end
    ...
    for (k, E_d) in enumerate(submatrices)
        val, dvals = partial_derivatives!(exprs, E_d, P, T, nvar - 1, nterm, access_input)
        push!(values, val)
        push!(dvalues, dvals)
...
function term(E::AbstractMatrix, P, ::Type{T}, nterm, access_input) where T
    @assert size(E, 2) == 1
    c = term_coefficient_with_parameters(T, P, nterm, access_input)
...
"""
function term_coefficient_with_parameters(T, P, nterm, access_input)
    monomial_product(T, P[:,nterm], :(c[$nterm]), access_input=access_input)[1]
end
term_coefficient_with_parameters(T, ::Nothing, nterm, access_input) = :(c[$nterm])
...
"""
function univariate!(exprs, E::AbstractMatrix, P, ::Type{T}, nvar, nterm, access_input) where T
    @assert size(E, 1) == 1
    n = size(E, 2)
    ...
    coeffs, E_filtered = coefficients_with_parameters!(exprs, E, P, T, nterm, access_input)
...
"""
function coefficients_with_parameters!(exprs, E, ::Nothing, ::Type{T}, nterm, access_input) where T
    n = size(E, 2)
    [:(c[$j]) for j=nterm:nterm+n-1], vec(E)
end
function coefficients_with_parameters!(exprs, E, P, ::Type{T}, nterm, access_input) where T
    n = size(E, 2)
    coeffs = []
    last = 1
    j = 1
    ...
    while j ≤ n
        ...
        if nduplicates == 0 # nothing happened
            c, _ = monomial_product(T, P[:,nterm_sub], :(c[$nterm_sub]), access_input=access_input)
            push!(coeffs, c)
        else
            ...
            c = generate_evaluate!(exprs, P_sub, T, size(P, 1), nterm_sub, access_input)
            push!(coeffs, c)
        end
        last = j
    end
